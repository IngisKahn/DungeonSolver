...
Setup[] setups =
    {
        ...
        new(new[] { 6, 2, 4, 3, 4, 4, 2, 6 }, new[] { 6, 2, 5, 3, 2, 5, 2, 6 }, new[] { (3, 1), (5, 1), (8, 3), (1, 4), (8, 5), (1, 6), (4, 8), (6, 8) }, Array.Empty<(int, int)>()),
        new(new[] { 2, 4, 4, 3, 2, 3, 4, 2 }, new[] { 0, 7, 2, 4, 2, 2, 7, 0 }, new[] { (1, 1), (8, 1), (2, 3), (8, 3), (7, 6), (1, 8), (8, 8) }, Array.Empty<(int, int)>()),
        ...
        new(new[] { 1, 5, 1, 4, 3, 5, 4, 3 }, new[] { 1, 3, 3, 6, 0, 6, 3, 4 }, new[] { (7, 1), (1, 7), (5, 7), (6, 8) }, new[] { (3, 3) })
    };

SolvePuzzle(new(setups[7]), true);
...
static bool CheckForCompletedLines(Board b, bool showOutput)
{
    for (var i = 1; i <= 8; i++)
    {
        ...
        if (b.RowCounts[i - 1] - b.CountRow(i, CellState.Wall) != b.CountRow(i, CellState.Unknown) ||
            b.CountRow(i, CellState.Unknown) <= 0) 
            continue;
        for (var j = 1; j <= 8; j++)
                if (b.State[j, i] == CellState.Unknown)
